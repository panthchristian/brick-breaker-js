...
function Tokens() {
  for (token in tokenLetters) {
    ...
    tokenSpeed = 10;
    //hit bottom
    if (tokenLetters[token].y > HEIGHT) {
      delete tokenLetters[token];
      break;
    }
    //collect
    if (
      tokenLetters[token].y + tokenSpeed > HEIGHT - paddleh &&
      tokenLetters[token].x - 5 > paddlex &&
      tokenLetters[token].x + 5 < paddlex + paddlew
    ) {
    ...
    if (tokenLetters[token]) {
      ...
      switch (tokenLetters[token].type) {
        case "B":
          tokenColor = "#000";
          changedTokenColor = "#e6bb45";
          break;
        case "W":
          tokenColor = "#000";
          changedTokenColor = "#050";
          break;
        case "3":
          tokenColor = "#000";
          changedTokenColor = "#0CC";
          break;
        case "F":
          tokenColor = "#bd0a01";
          changedTokenColor = "#000";
          break;
        case "S":
          tokenColor = "#bd0a01";
          changedTokenColor = "#ffffff";
          break;
      }

      tokenLetters[token].color % 7 !== 0
        ? tokenColor
...
function bounce() {
  ...
  for (i = 0; i < n_brick_rows; i++) {
    for (j = 0; j < n_brick_cols; j++) {
      ...
      var brick_x_r =
        brick_width + brick_padding + j * (brick_width + brick_padding);
      var brick_y_b =
        brick_padding + brick_height + i * (brick_height + brick_padding);
      var brick_y_t = brick_padding + i * (brick_height + brick_padding);
      var brick_edge = brick_height * 0.4;
      var tokenY = brick_y_b;
      var tokenX =
        0.5 * brick_width + brick_padding + j * (brick_width + brick_padding);
      ...
      if (bricks[i][j] == 1) {
        for (ball in balls) {
          ...
          if (
            ...
            y + dy - radius <= brick_y_b &&
            y + dy + radius >= brick_y_t &&
            x + dx - radius >= brick_x_l &&
            x + dx - radius <= brick_x_r
          ) {
    ...
    for (ball in balls) {
      ...
      if (y + dy > HEIGHT - radius - paddleh && lostCount == 0) {
        if (
          x + dx > paddlex - radius * 2 &&
          x + dx < paddlex + paddlew + radius * 2
        ) {
          //below surface or on edge
          if (
            HEIGHT - y - dy < paddleh ||
            x + dx < paddlex + paddlew * 0.1 + radius * 2 ||
            x + dx > paddlex + paddlew * 0.9 - radius * 2
          ) {
...
function toggleSound() {
  if (document.getElementById("sound").muted) {
    ...
    $("#togglesound").css("background-image", "url(./images/volume.svg)");
  } else {
    document.getElementById("sound").muted = true;
    document.getElementById("tokenSound").muted = true;
    $("#togglesound").css("background-image", "url(./images/volume-slash.svg)");
  }
}
