...
function intro() {
  ...
  $("#togglesoundintro")
    .show(500)
    .click(toggleintroSound)
    .delay(4200)
    .animate({ top: 40, left: 40 }, 700);
  $("#sup_head").delay(1000).animate({ marginTop: 100, opacity: 1 }, 400);
  $("#main_head")
    .delay(1900)
    .animate({ width: 643, height: 117, marginTop: 0, opacity: 1 }, 800);

  window.setTimeout(function () {
    $("#sup_head").animate(
      { width: 192.5, height: 41, marginTop: 10 },
      500,
      "linear"
    );
...
function toggleintroSound() {
  if (document.getElementById("introSound").muted) {
    ...
    $("#togglesoundintro").css("background-image", "url(./images/volume.svg)");
  } else {
    document.getElementById("introSound").muted = true;
    $("#togglesoundintro").css(
      "background-image",
      "url(./images/volume-slash.svg)"
    );
  }
}
...
function clickToStart() {
  $("#ctg").animate({ left: 352 }, 300, function () {
    $("#ctg").animate({ left: 355 }, 500, clickToStart());
  });
}
